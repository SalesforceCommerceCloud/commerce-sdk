 ...
 * Usage: ts-node examples/05-slas-helper-private.ts
...
// GUEST LOGIN
const guestTokenResponse = await helpers.loginGuestUserPrivate(
    slasClient, 
    { clientSecret: CLIENT_SECRET }
).catch(error => console.log("Error fetching token for guest login: ", error));

// REGISTERED B2C USER LOGIN
const registeredUserTokenResponse = await helpers.loginRegisteredUserB2Cprivate(
    slasClient,
    {
...
// // REFRESH TOKEN
const refreshTokenResponse = await helpers.refreshAccessTokenPrivate(
    slasClient, 
    { clientSecret: CLIENT_SECRET },
