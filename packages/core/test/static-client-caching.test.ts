...
describe("base client get test", () => {
  ...
  it("does not get from cache with request max-age=0", async () => {
    const scope = nock("https://somewhere")
      .get("/max-age-zero")
    ...
    let data = await _get({ client: client, path: "/max-age-zero" });
    expect(data).to.eql({ mock: "data" });
    data = await _get({ client: client, path: "/max-age-zero" });
