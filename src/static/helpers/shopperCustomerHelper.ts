...
type AuthFuncParamType = Parameters<sdk.Customer.ShopperCustomers["authorizeCustomer"]>[0];
// Type returned by the auth function
type CustomerType = sdk.Customer.ShopperCustomers.Customer;
...
export async function getShopperToken (
    ...
    let client = new sdk.Customer.ShopperCustomers(clientConfig);

    const response = await client.authorizeCustomer({body}, true);
    if (!response.ok) {
